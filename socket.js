const Define = require('./public/js/Define');
const { Character } = require('./public/js/Character');
const users = {};

module.exports = (io) => {
    io.on('connection', (socket) => {
        io.to(socket.id).emit('setting config', Define.CONFIG());
        io.to(socket.id).emit('existing users', users);
        // 기존 소켓에 count가 0보다 크면 count를 1증가
        // 아니면 새로운 유저 생성 유저생성이 되면 type= 'object'를 return 받음
        // const custom = ((count(socket, 0) > 0) ? count(socket, 1) : newUser(socket));
        const user = new Character({
            id: socket.id,
            displayName: `user ${Object.keys(users).length}`
        });
        users[user.id] = user;
        io.to(socket.id).emit('mine user', user);
        io.emit('enter user', user);

        socket.on('disconnect', () => {
            const id = socket.request.connection.remoteAddress;
            io.emit('Out user', users[id].displayName);
            delete users[id];
        });
        socket.on('send message', (text) => { // 3-3
            const msg = text;
            const id = socket.request.connection.remoteAddress;
            io.emit('receive message', id, msg);
        });
        socket.on('move user', (px) => { // 3-3
            const id = socket.request.connection.remoteAddress;
            const x = px[0];
            const y = px[1];
            if (!id) return;
            users[id].x = x;
            users[id].y = y;
            socket.broadcast.emit('receive move user', id, [x, y]);
        });
    });
};
...
// const server_ip = '192.168.100.31';
// const ip = [new Array(), new Array()];// 0= 아이피 주소, 1=아이피 소켓 갯수
...
//         } else {
//             const id = socket.request.connection.remoteAddress;
//             const index = ip[0].indexOf(id);
//             io.to(socket.id).emit('My user', ip[1][index]);
//         }
//         socket.on('disconnect', () => {
//             if (!((count(socket, -1) > 0))) {
//                 deleteUser(socket);
...
//             io.emit('receive message', id, msg);
//         });
//         socket.on('change name', (name) => { // 3-3
//             const id = socket.request.connection.remoteAddress;
//             const index = ip[0].indexOf(id);
...
//     // console.log();
//     const id = socket.request.connection.remoteAddress;
//     const u = new ChatUser(id, `u${R(456, 98783)}`);
//     ip[0].push(id);
//     ip[1].push(u);
//     // console.log(ip);
//     return u;

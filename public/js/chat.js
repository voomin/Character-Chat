...
document.onkeydown = function (event) {
    ...
    if (_MINE.spd <= _MINE.Maxspd) { _MINE.spd += _MINE.a; }
};
...
function move () {
    const spd = (typeof (_MINE.spd) === 'number') ? _MINE.spd : 0;
    if (_MINE.WENS[0]) {
        _MINE.px[0] -= spd;
    }
    if (_MINE.WENS[1]) {
    ...
    if (_MINE.WENS[0] || _MINE.WENS[1] || _MINE.WENS[2] || _MINE.WENS[3]) { socket.emit('move user', _MINE.px); }
}
